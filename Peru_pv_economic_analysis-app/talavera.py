...
def PWCO(pvi, pvom, N, T, Xd, Nd, Xl, Xec, Xis, il, dec, Nis, Nl):
    ...
    PVec = Xec * pvi
    PVis = Xis * pvi
    # loan_term = (PVl * il * (1 - T)) / (1 - (1 + il * (1 - T)) ** (-Nl)) * (q * (1 - q ** Nl)) / (1 - q) if Xl > 0 else 0
    ...
    # pvi = loan_term + equity_term + dividend_term + subsidy_term
...
def build_cashflow_series(pvi, epvs, epvg, ps, pg, rps, rpg, rd, d, N, T, pvom):
    flows = [-pvi]
    for n in range(1, N+1):
        ...
        income = (inflation_ps * energy_self + inflation_pg * energy_grid) * (1 - T)
        opex = pvom * ((1 + rom) ** (n - 1)) * (1 - T)
        net = income - opex
        flows.append(net)
    return flows
...
def run_simulation():
    ...
    cashflows = build_cashflow_series(pvi, epvs, epvg, ps, pg, rps, rpg, rd, d, N, T, pvom)
...
def plot_lcoe_vs_tariff(lcoe_talavera, base_tariff, rps, start_year=2018, N=25):
    years = np.arange(start_year, start_year + N)
    tariff_projection = base_tariff * (1 + rps) ** (years - start_year)

    plt.figure(figsize=(10, 6))
    plt.plot(years, [lcoe_talavera] * len(years), label="LCOE (Talavera)", linestyle="--", color="blue")
